...
class Row:
	...
	def __init__(self, s, ident, weight, sp, isT,comment=''):
...
class BinaryMatrix:
	...
	def from_file(self, file_name):
		...
		while line != '':
			if line != '\n':			
				...
				#endwhile
				if X == -1:
					self.add_row(set(content), ident, weight, sp, isT,comment)					
				else:
	...
	# isT: True if the row is a telomere - bool
	def add_row(self, row, ident = '-1', weight = Decimal('0'), sp = [], isT = False,comment=None):
		if ident == '-1':
			ident = str(self._height)
		#endif
	
		self._rows.append(Row(row, ident, weight, sp, isT,comment))
